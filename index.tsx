...
// --- CONSTANTES ---
const STORAGE_KEY = 'taskflow_v12_stable';
...
const App = () => {
  ...
  const [syncStatus, setSyncStatus] = useState<'online' | 'offline' | 'error' | 'syncing'>('offline');
  const [copied, setCopied] = useState(false);

  const isUpdatingFromCloud = useRef(false);
  const autoSaveTimer = useRef<any>(null);

  // 1. Cargar datos locales al iniciar
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.cloudId) setCloudId(parsed.cloudId);
      } catch (e) { console.error("Error cargando local storage", e); }
    }
    setDataLoaded(true);
  }, []);

  // 2. Función para subir a la nube
  const pushToCloud = async (id: string, payload: any) => {
    if (!id || isUpdatingFromCloud.current) return;
    setSyncStatus('syncing');
    try {
      const response = await fetch(`https://api.npoint.io/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...payload, lastUpdate: Date.now() })
      });
      if (response.ok) setSyncStatus('online');
  ...
  // 3. Función para descargar de la nube
  const fetchFromCloud = useCallback(async (id: string) => {
    if (!id || syncStatus === 'syncing') return;
    try {
      const response = await fetch(`https://api.npoint.io/${id}`);
      ...
      if (cloudData && cloudData.users) {
        // Marcamos que la actualización es interna para no disparar un "push" infinito
        ...
        if (JSON.stringify(cloudData.tasks) !== JSON.stringify(tasks)) setTasks(cloudData.tasks);
        if (JSON.stringify(cloudData.businessHours) !== JSON.stringify(businessHours)) setBusinessHours(cloudData.businessHours);
  ...
  }, [syncStatus, users, tasks, businessHours]);

  // 4. Crear nueva base de datos
  const createNewCloudId = async () => {
    setSyncStatus('syncing');
    try {
      const payload = { users, tasks, businessHours, lastUpdate: Date.now() };
      ...
      if (id) {
        setCloudId(id);
        setSyncStatus('online');
        alert(`¡Base de Datos Creada!\nID: ${id}\n\nComparte este ID con tu equipo para que todos vean lo mismo.`);
      } else {
        throw new Error("No se recibió ID");
      }
    } catch (e) {
      setSyncStatus('error');
      alert('Error de conexión. npoint.io podría estar saturado. Intenta de nuevo en unos segundos.');
    }
  };

  // 5. Guardado Automático y Persistencia
  useEffect(() => {
    if (!dataLoaded) return;

    // Guardar siempre en local primero
    const localData = { users, tasks, businessHours, cloudId };
    localStorage.setItem(STORAGE_KEY, JSON.stringify(localData));
    
    // Si hay nube, programar subida
    if (cloudId && currentUser && !isUpdatingFromCloud.current) {
      if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current);
      autoSaveTimer.current = setTimeout(() => {
        pushToCloud(cloudId, { users, tasks, businessHours });
      }, 4000); // 4 segundos de espera para no saturar la API
    }
  }, [users, tasks, businessHours, cloudId, currentUser, dataLoaded]);

  // 6. Polling para cambios externos
  useEffect(() => {
    if (cloudId && dataLoaded) {
      fetchFromCloud(cloudId);
      const interval = setInterval(() => fetchFromCloud(cloudId), 30000); // Cada 30 seg
      return () => clearInterval(interval);
    }
  }, [cloudId, dataLoaded, fetchFromCloud]);
        ...
        <div className="max-w-md w-full bg-white/5 backdrop-blur-3xl rounded-[48px] border border-white/10 shadow-2xl overflow-hidden p-12 text-center relative z-10">
          ...
          <div className="mt-12 pt-8 border-t border-white/5 space-y-4">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Unirse a Equipo</p>
            </div>
            <input 
              ...
              placeholder="PEGA EL ID DE EQUIPO AQUÍ"
              value={cloudId}
              onChange={e => setCloudId(e.target.value)}
            />
            <p className="text-[9px] text-slate-600 font-bold">Si ya tienes un ID, ingrésalo para cargar los datos del equipo.</p>
          </div>
        </div>
      </div>
    );
      ...
      <main className="flex-1 overflow-y-auto p-12 no-scrollbar bg-slate-50">
        <header className="flex justify-between items-center mb-16">
          ...
          <div className="flex items-center gap-6">
            <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl bg-white border border-slate-100 shadow-sm transition-all ${syncStatus === 'error' ? 'border-red-200 bg-red-50' : syncStatus === 'syncing' ? 'border-indigo-100 animate-pulse' : ''}`}>
               {syncStatus === 'online' && <Wifi size={16} className="text-emerald-500" />}
               {syncStatus === 'syncing' && <RefreshCw size={16} className="text-indigo-500 animate-spin" />}
               {syncStatus === 'offline' && <WifiOff size={16} className="text-slate-300" />}
               {syncStatus === 'error' && <AlertCircle size={16} className="text-red-500" />}
               <div className="text-left">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Estado Nube</p>
                  <p className={`text-[10px] font-black uppercase ${syncStatus === 'error' ? 'text-red-600' : 'text-slate-900'}`}>
                    {syncStatus === 'online' ? 'Sincronizado' : syncStatus === 'syncing' ? 'Guardando...' : syncStatus === 'error' ? 'Error Conexión' : 'Modo Local'}
                  </p>
               </div>
            </div>
        ...
        {view === 'settings' && (
          <div className="max-w-4xl space-y-12">
            <div className="bg-[#0f172a] p-12 rounded-[56px] shadow-2xl text-white relative overflow-hidden">
               ...
               <h3 className="text-3xl font-black mb-4 flex items-center gap-4"><Cloud size={40} className="text-indigo-400"/> Sistema de Nube</h3>
               <p className="text-slate-400 font-medium mb-12 max-w-xl">
                 La sincronización automática permite que todo tu equipo vea los cambios al instante. Si la nube falla, los datos se guardarán localmente y se subirán cuando haya conexión.
               </p>

               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Conectar ID Existente</p>
                    <div className="flex gap-4">
                      <input 
                        className="flex-1 bg-white/5 border border-white/10 rounded-3xl px-8 py-5 text-white font-bold outline-none focus:ring-1 focus:ring-indigo-500 tracking-widest uppercase"
                        placeholder="ID DE TU EQUIPO"
                        value={cloudId}
                        onChange={e => setCloudId(e.target.value)}
                      />
                      <button onClick={() => fetchFromCloud(cloudId)} className="bg-indigo-600 text-white px-8 rounded-3xl font-black hover:bg-indigo-500 transition-all shadow-lg"><RefreshCw size={20}/></button>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Crear Nueva Nube</p>
                    <button onClick={createNewCloudId} disabled={syncStatus === 'syncing'} className="w-full bg-white text-slate-900 py-5 rounded-3xl font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3 hover:bg-slate-100 transition-all disabled:opacity-50">
                      <Plus size={18}/> {syncStatus === 'syncing' ? 'CREANDO...' : 'GENERAR NUEVO ID'}
                    </button>
                  </div>
               </div>
               ...
               {cloudId && (
                 <div className="mt-12 p-8 bg-indigo-600/20 rounded-[40px] border border-indigo-500/30 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] mb-1">ID para compartir:</p>
            ...
            <div className="bg-white p-12 rounded-[56px] shadow-sm border border-slate-100">
              <h3 className="text-2xl font-black mb-8 flex items-center gap-4"><Clock className="text-indigo-600"/> Horarios de Trabajo</h3>
  ...
  return (
    <form className="space-y-6" onSubmit={e => { e.preventDefault(); onLogin(u, p); }}>
      <select className="w-full px-8 py-5 bg-white/5 border border-white/10 rounded-2xl text-white font-bold outline-none cursor-pointer appearance-none text-center" value={u} onChange={e => setU(e.target.value)} required>
        <option value="" disabled className="bg-[#0a0f1d]">-- Selecciona Usuario --</option>
        {users.map((usr: any) => <option key={usr.id} value={usr.username} className="bg-[#0a0f1d]">{usr.name}</option>)}
      </select>
      <input type="password" placeholder="Contraseña" className="w-full px-8 py-5 bg-white/5 border border-white/10 rounded-2xl text-white font-bold outline-none focus:ring-1 focus:ring-indigo-500 text-center" value={p} onChange={e => setP(e.target.value)} required />
      <button className="w-full bg-indigo-600 text-white font-black py-5 rounded-2xl shadow-xl hover:bg-indigo-500 transition-all active:scale-95 shadow-indigo-600/20 uppercase tracking-widest text-xs">INGRESAR AL SISTEMA</button>
    </form>
  );
};
...
const DashboardView = ({ tasks, users, hours, role }: any) => {
  const stats = [
    ...
    { label: 'En Curso', val: tasks.filter((t:any)=>t.status==='accepted').length, color: 'bg-indigo-600' },
    { label: 'Finalizadas', val: tasks.filter((t:any)=>t.status==='completed').length, color: 'bg-emerald-500' },
  ];
        ...
        {tasks.map((t: Task) => (
          <div key={t.id} className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm flex flex-col group hover:shadow-xl transition-all duration-500">
             <div className="flex justify-between items-start mb-6">
               <div className="flex-1">
                  <div className="flex items-center gap-4 mb-4">
                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${t.status === 'completed' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : t.status === 'accepted' ? 'bg-indigo-50 text-indigo-600 border border-indigo-100' : 'bg-amber-50 text-amber-600 border border-amber-100'}`}>
                      {t.status === 'completed' ? 'Completada' : t.status === 'accepted' ? 'En ejecución' : 'En espera'}
                    </span>
               ...
               {role === 'admin' && <button onClick={() => confirm('¿Borrar tarea?') && onDelete(t.id)} className="text-slate-200 hover:text-red-500 p-2 transition-all hover:bg-red-50 rounded-2xl"><Trash2/></button>}
             </div>
             
             {role === 'user' && t.status !== 'completed' && (
               <div className="mt-8 pt-8 border-t flex flex-wrap gap-4">
                 {t.status === 'pending' ? (
                   <button onClick={() => onUpdate(t.id, {status: 'accepted', acceptedAt: Date.now()})} className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all shadow-indigo-600/20">INICIAR TRABAJO</button>
                 ) : (
                   <div className="flex-1 flex gap-4 min-w-[350px]">
                      <input className="flex-1 px-6 py-4 border border-slate-100 rounded-2xl font-bold outline-none focus:ring-1 focus:ring-indigo-500" placeholder="Escribe un avance..." value={note} onChange={e => setNote(e.target.value)} />
             ...
             {t.notes.length > 0 && (
               <div className="mt-8 space-y-2">
                 <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest ml-2">Reportes del colaborador</p>
                 {t.notes.map((n:any)=>(
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl flex items-center justify-center p-8 z-[100] animate-in fade-in duration-300">
          <div className="bg-white p-12 rounded-[56px] w-full max-w-md shadow-2xl relative">
             ...
             <h3 className="text-3xl font-black mb-8 tracking-tighter text-slate-900">Nueva Tarea</h3>
             <div className="space-y-4">
                <input placeholder="Título" className="w-full px-8 py-5 bg-slate-50 border border-slate-100 rounded-3xl font-bold outline-none focus:ring-2 focus:ring-indigo-500" value={f.title} onChange={e => setF({...f, title: e.target.value})} />
                <textarea placeholder="Descripción detallada..." className="w-full px-8 py-5 bg-slate-50 border border-slate-100 rounded-3xl font-medium h-32 outline-none resize-none focus:ring-2 focus:ring-indigo-500" value={f.description} onChange={e => setF({...f, description: e.target.value})} />
             ...
             <button onClick={() => { if(f.title && f.assignedTo) { onAdd(f); setShowModal(false); } else alert('Faltan datos.'); }} className="w-full bg-indigo-600 text-white py-6 rounded-3xl font-black shadow-xl mt-8 uppercase tracking-widest text-sm hover:bg-indigo-700 active:scale-95 transition-all">ASIGNAR TAREA</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <h3 className="text-2xl font-black uppercase tracking-widest text-slate-400">Equipo</h3>
        <button onClick={() => { setModal({open:true, editing:null}); setF({name:'',username:'',password:'',role:'user'}); }} className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-lg hover:bg-indigo-700 transition-all active:scale-95">
        ...
        {users.map((u:User) => (
          <div key={u.id} className="bg-white p-10 rounded-[56px] border border-slate-100 text-center flex flex-col items-center relative group shadow-sm transition-all hover:border-indigo-200">
            <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button onClick={() => confirm('¿Borrar usuario?') && onDelete(u.id)} className="p-3 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={16}/></button>
            </div>
      ...
      {modal.open && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl flex items-center justify-center p-8 z-[100] animate-in fade-in duration-300">
          <div className="bg-white p-12 rounded-[56px] w-full max-w-md shadow-2xl relative">
             ...
             <h3 className="text-3xl font-black mb-8 tracking-tighter text-slate-900">{modal.editing ? 'Editar' : 'Crear'} Perfil</h3>
             <div className="space-y-4">
                <input placeholder="Nombre" className="w-full px-8 py-5 bg-slate-50 border border-slate-100 rounded-3xl font-bold outline-none" value={f.name} onChange={e => setF({...f, name: e.target.value})} />
                <input placeholder="Usuario" className="w-full px-8 py-5 bg-slate-50 border border-slate-100 rounded-3xl font-bold outline-none" value={f.username} onChange={e => setF({...f, username: e.target.value})} />
             ...
             <button onClick={handleSave} className="w-full bg-indigo-600 text-white py-6 rounded-3xl font-black mt-10 shadow-xl shadow-indigo-600/30 uppercase text-sm tracking-widest transition-all">GUARDAR</button>
          </div>
        </div>
      )}
    </div>
