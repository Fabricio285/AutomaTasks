...
const App = () => {
  ...
  const initialData = loadInitial();

  const [users, setUsers] = useState<User[]>(initialData?.users || [DEFAULT_ADMIN]);
  const [tasks, setTasks] = useState<Task[]>(initialData?.tasks || []);
  const [businessHours, setBusinessHours] = useState<BusinessHours>(initialData?.hours || INITIAL_HOURS);
  const [cloudId, setCloudId] = useState<string>(initialData?.cloudId || '');
          ...
          <div className="flex items-center gap-6">
            <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl bg-white border border-slate-100 shadow-sm transition-all ${lastSyncStatus === 'error' ? 'border-red-200 bg-red-50' : ''}`}>
               ...
               <div className="text-left">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Cloud Status</p>
                  <p className="text-[10px] font-black text-slate-900">{cloudId ? `CONECTADO` : 'MODO LOCAL'}</p>
               </div>
            </div>
            ...
            <div className="bg-[#0f172a] p-12 rounded-[56px] shadow-2xl text-white relative overflow-hidden">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Vincular ID Existente</p>
               ...
               {cloudId && (
                 <div className="mt-12 p-8 bg-indigo-600/20 rounded-[40px] border border-indigo-500/30 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] mb-1">ID Compartible:</p>
               ...
               <div className="mt-8 pt-8 border-t flex flex-wrap gap-4">
                 {t.status === 'pending' ? (
                   ...
                   <div className="flex-1 flex gap-4 min-w-[350px]">
                      <input className="flex-1 px-6 py-4 border border-slate-100 rounded-2xl px-6 py-4 font-bold outline-none focus:ring-1 focus:ring-indigo-500" placeholder="Añade un reporte de avance..." value={note} onChange={e => setNote(e.target.value)} />
